                ...
                while (true)
                {
                    switch (task.IsCompleted)
                    {
                        case true when !sendDictionary.ContainsKey(task.Result):
                            sendDictionary[task.Result] = task.Result.SendAsync(new ArraySegment<byte>(),SocketFlags.None);
                    ...
                    {
                        socket.SendAsync(new ArraySegment<byte>(), SocketFlags.None);
                        if (!forReceive.ContainsKey(socket))
                        {
                            clientData.Add(socket, new ArraySegment<byte>());
                    ...
                    {
                        Console.WriteLine(clientData[socket]);
                        if (socket.Blocking)
                        {
